...
| --- | --- | --- | --- | --- |
| C1 | Teszt, amely rögzíti a fenti `surface` szabályt (CLI ↔ IDE értékek nem keveredhetnek) | `cli/tests`, `vscode-extension/test` | — | ⬜ |
| C2 | `latent status`: holding tier, aznapi tényleges szorzó, napi boost plafon | `cli/src/index.ts` | szerver tier endpoint (E7.5) | ⬜ |
| C3 | `protocol/openapi.yaml`: új publikus végpontok (block statisztika, tier, Spotlight), amint a szerveren élnek | `protocol/` | szerver E3/E8 | ⬜ |
| C4 | Hálózat: Base → Robinhood Chain (4663), ha DEC-1 így dönt. Az EVM kulcs és a cím változatlan; csak a hálózati config és a README szövege | `cli/src/config.ts`, README | DEC-1 | ⬜ |
| C5 | Mock szerver (`tools/`) bővítése az új végpontokkal, hogy a kliens hálózat nélkül fejleszthető maradjon | `tools/` | C3 | ⬜ |
