...
export function handleAdsCommand(
  ...
  if (cmd === "on" || cmd === "off") {
    ...
    if (!allowed) return { text: "Only the owner of this agent can turn sponsored footers on or off." };
...
export function registerAdsCommand(api: OpenClawPluginApi, config: PluginConfig, state: AdsState): void {
  api.registerCommand({
    name: "ads",
