...
import { getConfig, type PluginConfig } from "./lib/config.js";
import { registerReplyFooter, type AdsState } from "./hooks/reply-footer.js";
...
export default definePluginEntry({
  ...
  description:
    "Earn USDC from a labelled sponsored footer under the agent's final reply. " +
    "Open ad marketplace for AI agents on Base.",

  register(api) {
    // `api.config` is the whole OpenClaw config; ours is `api.pluginConfig`.
    const config = getConfig((api.pluginConfig ?? {}) as Partial<PluginConfig>);
